		...
		if(goodjet_n >= 4)
		  {
		    ...
		    if(goodbjet_n >= 1)   // In the Zprime analysis we consider 1 bjet
		      {
			...
			if(met_et > 30000.)
              ...
              TLorentzVector bjet1  = TLorentzVector();
              TLorentzVector bjet2  = TLorentzVector();

              TLorentzVector sum_particles  = TLorentzVector();
              TLorentzVector sum_jets  = TLorentzVector();
              TLorentzVector sum_jets3 = TLorentzVector();

              jet1.SetPtEtaPhiE(jet_pt[goodjet_index[0]], jet_eta[goodjet_index[0]], jet_phi[goodjet_index[0]],jet_E[goodjet_index[0]]);
              jet2.SetPtEtaPhiE(jet_pt[goodjet_index[1]], jet_eta[goodjet_index[1]], jet_phi[goodjet_index[1]],jet_E[goodjet_index[1]]);
              bjet1.SetPtEtaPhiE(jet_pt[goodjet_index[2]], jet_eta[goodjet_index[2]], jet_phi[goodjet_index[2]], jet_E[goodjet_index[2]]);
              bjet2.SetPtEtaPhiE(jet_pt[goodbjet_index[0]], jet_eta[goodbjet_index[0]], jet_phi[goodbjet_index[0]], jet_E[goodbjet_index[0]]);
              sum_particles.SetPxPyPzE(0,0,0,0);
              sum_jets.SetPxPyPzE(0,0,0,0);
              sum_jets3.SetPxPyPzE(0,0,0,0);

              sum_jets = jet1 + jet2 ;
              sum_jets3 = jet1 + jet2 + bjet1;
              sum_particles = Lepton_1 + MeT + jet1 + jet2 + bjet1 + bjet2;
              // sum_particles.Mt(); // object transverse mass



      				double names_of_global_variable[]={InvMass1_inGeV, missingEt, vxp_z, (double)pvxp_n, Lepton1_MeT_MT/1000., sum_particles.M()/1000., sum_jets.M()/1e6, sum_jets3.M()/1000.};
      				...
      				TString histonames_of_global_variable[]={"hist_vismass","hist_etmiss","hist_vxp_z","hist_pvxp_n", "hist_mt", "hist_mT", "hist_mj", "hist_mj3"};
