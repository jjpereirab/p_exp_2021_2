...
void TopAnalysis::define_histograms()
{
  ...
  hist_etmiss       = new TH1F("hist_etmiss",      "Missing Transverse Momentum;E_{T,Miss} [GeV];Events", 20, 0,600);
  ...
  hist_leadjet_pt       = new TH1F("hist_jet_pt",      "Jet Transverse Momentum;p_{T}^{jet} [GeV];Jets", 20, 0, 600);
  ...
  hist_leadleptpt   = new TH1F("hist_leadleptpt",  "Leading Lepton Transverse Momentum;p_{T}^{leadlep} [GeV];Leptons", 30, 0, 600);
  ...
  hist_mj    = new TH1F("hist_mj",   "2 jets mass; m_{jets} (TeV); Events", 30, 0, 2.);
  hist_mj3    = new TH1F("hist_mj3",   "3 jets mass; m_{jets}; Events", 20, 0, 3000);
}
...
void TopAnalysis::FillHistogramsGlobal( double m, float w , TString s)
{
  ...
  if (s.Contains("hist_mj3")) hist_mj3->Fill(m,w);
